...
public interface StandardLibaryNodeMapper extends Mapper<StandardLibaryNode> {
    ...
    @Update("update evaluation_libary_node  set description=#{param2},name=#{param3} where exists (select 1 from applicability_libary_node a where evaluation_libary_node.applicability_node_id=a.id and a.standard_node_id=#{param1})")
